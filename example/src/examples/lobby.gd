...
func _join_Lobby(lobby_id: int) -> void:
	$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] Attempting to join lobby "+str(lobby_id)+"...\n")
...
func _leave_Lobby() -> void:
	...
	if LOBBY_ID != 0:
		...
		$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] Leaving lobby "+str(LOBBY_ID)+".\n")
...
func _on_Lobby_Created(connect_result: int, lobby_id: int) -> void:
	if connect_result == 1:
		$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] Created a lobby: "+str(LOBBY_ID)+"\n")
		...
		$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] Allowing Steam to be relay backup: "+str(IS_RELAY)+"\n")

		# Enable the leave lobby button and all testing buttons
		_change_Button_Controls(false)
	else:
		$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] Failed dto create lobby\n")
...
func _on_Lobby_Invite(inviter: int, lobby_id: int, game_id: int) -> void:
	$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] You have received an invite from "+str(Steam.getFriendPersonaName(inviter))+" to join lobby "+str(lobby_id)+" / game "+str(game_id)+"\n")
...
func _on_Lobby_Joined(lobby_id: int, _permissions: int, _locked: bool, response: int) -> void:
	...
	if response == 1:
		...
		$Frame/Main/Displays/Outputs/Titles/Lobby.set_text("Lobby ID: "+str(LOBBY_ID))
		# Append to output
		$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] Joined lobby "+str(LOBBY_ID)+".\n")
...
func _on_Lobby_Join_Requested(lobby_id: int, friend_id: int) -> void:
	...
	$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] Joining "+str(OWNER_NAME)+"'s lobby...\n")
...
func _make_P2P_Handshake() -> void:
	$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] Sending P2P handshake to the lobby...\n")
...
func _on_P2P_Session_Request(remoteID: int) -> void:
	...
	$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] P2P session request from "+str(REQUESTER)+"\n")
	# Accept the P2P session; can apply logic to deny this request if needed
	var SESSION_ACCEPTED: bool = Steam.acceptP2PSessionWithUser(remoteID)
	$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] P2P session was connected: "+str(SESSION_ACCEPTED)+"\n")
	# Make the initial handshake
	_make_P2P_Handshake()
...
func _read_P2P_Packet() -> void:
	...
	if PACKET_SIZE > 0:
		...
		$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] Packet from "+str(PACKET_SENDER)+": "+str(READABLE)+"\n")
...
func _send_P2P_Packet(target: int, packet_data: Dictionary) -> void:
	...
	$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] P2P packet sent successfully? "+str(SEND_RESPONSE)+"\n")
...
func _add_Player_List(steam_id: int, steam_name: String) -> void:
	...
	var THIS_SIGNAL: int = THIS_MEMBER.connect("kick_player", Callable(self, "_on_Lobby_Kick"))
...
func _on_Create_Lobby_pressed() -> void:
	...
	_create_Lobby()
	$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] Attempt to create a new lobby...\n")
...
func _on_Get_Lobby_Data_pressed() -> void:
	...
	$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] Lobby data, name: "+str(DATA)+"\n")
	DATA = Steam.getLobbyData(LOBBY_ID, "mode")
	$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] Lobby data, mode: "+str(DATA)+"\n")
...
func _on_Lobby_Match_List(lobbies: Array) -> void:
	...
	for LOBBY in lobbies:
		...
		var LOBBY_SIGNAL: int = LOBBY_BUTTON.connect("pressed", Callable(self, "_join_Lobby").bind(LOBBY))
...
func _on_Open_Lobby_List_pressed() -> void:
	...
	# Request the list
	$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] Requesting a lobby list...\n")
...
func _on_Lobby_Chat_Update(lobby_id: int, changed_id: int, making_change_id: int, chat_state: int) -> void:
	...
	# Else if a player has left the lobby
		...
		$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] "+str(CHANGER)+" has left the lobby.\n")
	# Else if a player has been kicked
	elif chat_state == 8:
		$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] "+str(CHANGER)+" has been kicked from the lobby.\n")
	# Else if a player has been banned
	elif chat_state == 16:
		$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] "+str(CHANGER)+" has been banned from the lobby.\n")
	# Else there was some unknown change
	else:
		$Frame/Main/Displays/Outputs/Output.append_text("[STEAM] "+str(CHANGER)+" did... something.\n")
...
func _on_Lobby_Message(_result: int, user: int, message: String, type: int) -> void:
	...
	if type == 1:
		...
		# Else this is just chat message
			...
			$Frame/Main/Displays/Outputs/Output.append_text(str(SENDER)+" says '"+str(message)+"'\n")
	# Else this is a different type of message
		...
		match type:
			...
			9: $Frame/Main/Displays/Outputs/Output.append_text(str(SENDER)+" was banned!\n")
			10: $Frame/Main/Displays/Outputs/Output.append_text(str(SENDER)+" disconnected.\n")
			11: $Frame/Main/Displays/Outputs/Output.append_text(str(SENDER)+" sent an old, offline message.\n")
			12: $Frame/Main/Displays/Outputs/Output.append_text(str(SENDER)+" sent a link that was removed by the chat filter.\n")
...
func _on_Send_Chat_pressed() -> void:
	...
	if MESSAGE.length() > 0:
		...
		var x1 = Steam.getServerDetails(0,0)
